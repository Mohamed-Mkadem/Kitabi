...
@push('title')
    <title>لوحة التحكّم - تعديل الكتاب</title>
@endpush
        ...
        <div class="starter-header d-flex a-center j-between " id="starter-header">
            <div class="greeting-holder">
                ...
                <x-breadcrumb class="dashboard" prevUrl="{{ route('admin.home') }}" prevValue="الرئيسية"
                    currUrl="{{ route('admin.books.edit', $book) }}" currValue="تعديل الكتاب" />
            </div>
        </div>
        <!-- End Starter Header -->

        <div class="holder pt-1 pb-1 pe-1 ps-1">
            <form action="{{ route('admin.books.update', $book) }}" method="post" class="form-grid"
                enctype="multipart/form-data" id="book-form" novalidate>
                @csrf
                @method('PATCH')
                <div class="row mb-1">
                    <div class="form-control">
                        ...
                        <input class="form-element" required type="text" name="name" id="name-input"
                            placeholder="اسم الكتاب" value="{{ $book->name }}">
                ...
                <div class="row mb-1">
                    <div class="form-control">
                        ...
                        <textarea id="description-input" name="description" placeholder="ضع وصفا مناسبا للكتاب هنا" required
                            class="form-element">{{ $book->description }}</textarea>
                    ...
                    <div class="form-control">
                        ...
                        <div class="statuses-holder ">
                            ...
                            <div class="status form-element">
                                ...
                                <input {{ $book->status == 'published' ? 'checked' : '' }} type="radio" id="published"
                                    name="status" value="published">
                            </div>

                            <div class="status form-element">
                                <label for="hidden">غير منشور</label>
                                <input {{ $book->status == 'hidden' ? 'checked' : '' }} type="radio" id="hidden"
                ...
                <div class="row mb-1 ">
                    <div class="form-control">
                        ...
                        <input class="form-element" type="number" id="cost-price-input" placeholder="مثال : 10"
                            name="cost_price" value="{{ $book->cost_price }}">
                ...
                <div class="row mb-1 ">
                    ...
                    <div class="form-control">
                        ...
                        <input class="form-element" type="number" id="stock-alert-input" placeholder="مثال : 10"
                            name="stock_alert" value="{{ $book->stock_alert }}">
                                ...
                                @foreach ($authors as $author)
                                    <div class="choice">
                                        <input type="radio" name="author_id" value="{{ $author->id }}"
                                            id="author-{{ $author->id }}"
                                            {{ $book->author_id == $author->id ? 'checked' : '' }}>
                                ...
                                @foreach ($publishers as $publisher)
                                    <div class="choice">
                                        <input type="radio" name="publisher_id" value="{{ $publisher->id }}"
                                            ...
                                            {{ $book->publisher_id == $publisher->id ? 'checked' : '' }}>
                ...
                <div class="d-flex a-center gap-1 f-wrap mt-2">
                    <button class="submitBtn" type="submit">تعديل</button>
